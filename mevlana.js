...
// github: @memrearal
(function(undefined) {
var AFTER, BEFORE, COMMON_EVENTS, EventEmitter, FETCH, FIRE, FormData, NativeFetch, NativeFormData, NativeXMLHttp, OFF, ON, READY_STATE, UPLOAD_EVENTS, WINDOW, MEVLANAFetchRequest, MEVLANAFormData, MEVLANAHttpRequest, XMLHTTP, convertHeaders, depricatedProp, document, fakeEvent, mergeObjects, msie, nullify, proxyEvents, slice, useragent, MEVLANA, _base,
...
MEVLANA = EventEmitter(true);

MEVLANA.EventEmitter = EventEmitter;

MEVLANA[BEFORE] = function(handler, i) {
  if (handler.length < 1 || handler.length > 2) {
    throw "invalid hook";
  }
  return MEVLANA[ON](BEFORE, handler, i);
};

MEVLANA[AFTER] = function(handler, i) {
  if (handler.length < 2 || handler.length > 3) {
    throw "invalid hook";
  }
  return MEVLANA[ON](AFTER, handler, i);
};

MEVLANA.enable = function() {
  WINDOW[XMLHTTP] = MEVLANAHttpRequest;
  if (typeof MEVLANAFetchRequest === "function") {
    WINDOW[FETCH] = MEVLANAFetchRequest;
  }
  if (NativeFormData) {
    WINDOW[FormData] = MEVLANAFormData;
  }
};

MEVLANA.disable = function() {
  WINDOW[XMLHTTP] = MEVLANA[XMLHTTP];
  WINDOW[FETCH] = MEVLANA[FETCH];
  if (NativeFormData) {
    WINDOW[FormData] = NativeFormData;
  }
};

convertHeaders = MEVLANA.headers = function(h, dest) {
...
if (NativeFormData) {
  MEVLANA[FormData] = NativeFormData;
  WINDOW[FormData] = MEVLANAFormData;
}

NativeXMLHttp = WINDOW[XMLHTTP];

MEVLANA[XMLHTTP] = NativeXMLHttp;

MEVLANAHttpRequest = WINDOW[XMLHTTP] = function() {
  ...
  if ('withCredentials' in xhr || MEVLANA.addWithCredentials) {
  ...
  facade.send = function(body) {
    ...
    send = function() {
      ...
      if (request.body instanceof MEVLANAFormData) {
        request.body = request.body.fd;
      }
      xhr.send(request.body);
    };
    hooks = MEVLANA.listeners(BEFORE);
    process = function() {
      var done, hook;
      if (!hooks.length) {
        return send();
...
if (typeof WINDOW[FETCH] === "function") {
  ...
  MEVLANA[FETCH] = NativeFetch;
  MEVLANAFetchRequest = WINDOW[FETCH] = function(url, options) {
    ...
    options.url = url;
    request = null;
    beforeHooks = MEVLANA.listeners(BEFORE);
    afterHooks = MEVLANA.listeners(AFTER);
    return new Promise(function(resolve, reject) {
      var done, getRequest, processAfter, processBefore, send;
      getRequest = function() {
        if (options.body instanceof MEVLANAFormData) {
...
MEVLANAHttpRequest.UNSENT = 0;

MEVLANAHttpRequest.OPENED = 1;

MEVLANAHttpRequest.HEADERS_RECEIVED = 2;

MEVLANAHttpRequest.LOADING = 3;

MEVLANAHttpRequest.DONE = 4;

if (typeof define === "function" && define.amd) {
  define("MEVLANA", [], function() {
    return MEVLANA;
  });
